...
import { reportInstructions } from './report.js';
...
function liftLine(f) {
  ...
  if (f.tempo) bits.push(`tempo ${f.tempo}`);
  if (f.tempoDrift) bits.push(`negative shortened ${f.tempoDrift.from}s -> ${f.tempoDrift.to}s`);
  if (f.ego) {
    bits.push(`WEIGHT OUTRUNNING STRENGTH: load +${f.ego.weightTrend}%/session, strength ${f.ego.strengthTrend}%/session`);
  }

  // Only when he actually scored it. An unscored lift says nothing here rather
  // than saying "0", which would read as the worst possible session.
  if (f.feel?.scored) {
    bits.push(
      `felt ${f.feel.latest}/5 last time (avg ${f.feel.average} over ${f.feel.scored})${f.feel.falling ? ', and getting worse' : ''}`,
    );
  }
  return `- ${bits.join(' · ')}`;
}
...
export function buildSummaryPrompt(findings) {
  ...
  const profiles = (f.profiles ?? [])
    .map((p) => `- ${p.label}: ${p.percentPerSession > 0 ? '+' : ''}${p.percentPerSession}%/session over ${p.sessions} sessions`)
    .join('\n');

  const groups = (f.groups ?? [])
    .map((g) => `- ${g.muscleGroup}: ${g.percentPerSession > 0 ? '+' : ''}${g.percentPerSession}%/session, ${g.profiles} profiles${g.machines ? ` across ${g.machines} machines` : ''}`)
    .join('\n');

  const history = (f.history ?? [])
    .map((h) => `- ${h.date} ${h.day ?? ''}${h.gym ? ` @ ${h.gym}` : ''}: ${h.sets} sets, ${h.volume} lb`)
    .join('\n');

  // Kept separate from the session check-ins: he can score sets without ever
  // filling in a check-in, and then this explanation still has to appear.
  const flaggedEgo = (f.lifts ?? []).some((x) => x.ego);
  const scoredLifts = (f.lifts ?? []).some((x) => x.feel?.scored);
  const perLift = scoredLifts
    ? [
        'Some lifts carry a per-set feel score, shown on their line above as "felt N/5".',
        'That is 1 awful to 5 strong, tapped on the set itself. A lift that feels worse while',
        'its numbers hold is the earliest warning in this data — say so if you see it.',
      ].join('\n')
    : '';
  ...
  return [
    ...
    ...(f.nutrition?.summary
      ? [
          'Bodyweight and intake. Read every trend below against this:',
          f.nutrition.summary,
          ...(f.nutrition.goal ? [`He says he is currently: ${f.nutrition.goal}.`] : []),
          ...(f.nutrition.deficitPerDay !== null && f.nutrition.deficitPerDay !== undefined
            ? [
                `The scale says his real deficit is about ${f.nutrition.deficitPerDay} kcal a day, which puts his actual daily burn near ${f.nutrition.impliedTDEE} kcal. That is measured from weight lost, not estimated, so trust it over any formula.`,
              ]
            : []),
          ...(f.nutrition.verdict ? [f.nutrition.verdict.message] : []),
          ...(f.nutrition.guidance ? [f.nutrition.guidance] : []),
          '',
        ]
      : []),
    'Per-lift verdicts:',
    ...
    ...(profiles
      ? [
          'Per-machine trends. A lift on two machines is two series, because the loads do not',
          'compare across stacks — this is where "got stronger" and "changed machine" separate:',
          profiles,
          '',
        ]
      : []),
    ...(groups ? ['Muscle groups, each profile indexed to its own baseline before combining:', groups, ''] : []),
    ...(history ? ['Every session logged, oldest first:', history, ''] : []),
    ...(perLift ? [perLift, ''] : []),
    ...(flaggedEgo
      ? [
          'A lift marked WEIGHT OUTRUNNING STRENGTH is adding load faster than its estimated',
          '1RM is rising — the extra weight is coming out of reps rather than strength. That is',
          'the most actionable thing in this payload; lead with it and say which lift.',
          '',
        ]
      : []),
    feel,
    ...
    '',
    'Rules:',
    '- Plain language. Address him as "you".',
    '- If the lifts genuinely have nothing in common, say that plainly rather than inventing a link.',
    '- You cannot see his form, his technique, or his video. Never claim to.',
...
export function summaryCacheKey(findings) {
  ...
  const shape = JSON.stringify({
    ...
    l: (f.lifts ?? []).map((x) => [
      x.name, x.status, x.percentPerSession, x.machine ?? null,
      // Scoring a set has to invalidate the cache too, or the card keeps a
      // summary written before he said the lift felt awful.
      x.feel?.latest ?? null, x.feel?.scored ?? null,
    ]),
